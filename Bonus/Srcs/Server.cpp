...
void	fileTransfer(std::string str, Network &nw, int index);
...
void Server::run() {
	...
	while (true) {			
		...
		for (int i = 1; i < this->nw.getConnections(); i++) {
			...
			if (this->bot.receiveAndProcessMessages(this->nw, this->users, i) == true) {
				this->nw.clearCach(i);
				continue;
			}
			this->matchCommands(this->nw.getCach(i), i);
			this->nw.clearCach(i);
		}
	}
}
...
bool Server::doPrivmsg(std::istringstream& iss, int index) {
	...
	if (name[0] == '#' || name[0] == '&')
	{
		...
		//we send it to the bot first
		if (ch.getBotEnabled())
			bot.replaceBadWords(data);
...
bool Server::doUser(std::istringstream& iss, int index) {
	...
	if (username == "BOT")
		throw std::runtime_error("Error: Username can not be 'BOT'\n");
	if (username.size() == 0)
...
bool Server::doJoin(std::istringstream& iss, int index) {
	...
	else
	{
		...
		ch.addUser(this->users[0]);
		this->nw.sendMsg(this->users[index - 1].getID(), "Bot joined to the Channel " + channelname + "\n");
	}
	return (true);
}
...
bool Server::doKick(std::istringstream& iss, int index) {
	...
	//cannot kick BOT
	if (username == "BOT")
		throw std::runtime_error("Error: You can't kick the Bot!\n");
...
bool Server::doMode(std::istringstream& iss, int index) {
	...
	{
		//enable or disable bot in a certain channel
		ch.toggleBot();
		if (ch.getBotEnabled())
		{
			nw.sendMsg(this->users[index - 1].getID(), "You enabled the Bot in this channel\n");
		}
		else
		{
			nw.sendMsg(this->users[index - 1].getID(), "You disabled the Bot in this channel\n");
		}
	}
